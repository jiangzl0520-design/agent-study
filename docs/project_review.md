# AI Agent Study Project 项目复盘：V1 - V9.2
...
V9.2 是在 V9 本地知识库问答 Agent 基础上的一次 RAG 检索能力升级。

V9 已经实现了从 `knowledge/knowledge.txt` 中读取知识库，并根据用户问题进行简单关键词搜索。V9.2 的目标不是引入复杂框架，而是在不使用向量数据库、不使用 embedding、不引入新依赖的前提下，让本地 RAG 更接近真实检索流程。
...
这样做的目的是先理解 RAG 的核心流程，而不是一开始就被复杂框架掩盖底层原理。
...
增加关键词提取
增加片段评分
改进 search_knowledge 返回格式
```

#### 1. 改进 split_knowledge()

V9 的文本切分比较简单，主要按空行分段。

V9.2 中，`split_knowledge()` 进一步改进为：

```text
先按空行切分段落
过滤空段落
如果段落较短，直接作为一个 chunk
如果段落太长，再按 max_chars 切成多个 chunk
切长文本时保留 overlap，避免上下文断裂
```
...
避免知识片段太长，影响检索准确度
避免知识片段太短，丢失上下文
通过 overlap 保留相邻片段之间的语义连接
```

这让我理解了 RAG 中“文档切片”的基本思想。

---

#### 2. 新增关键词提取逻辑

V9.2 增加了类似 `extract_keywords()` 的逻辑。
...
V9.2 增加了类似 `score_paragraph()` 的逻辑。

它的作用是：

```text
计算用户问题和知识库片段之间的相关性分数
```
...
这一步让我理解了 RAG 中“检索排序”的基础思想。

---

#### 4. 改进 search_knowledge()

V9.2 中，`search_knowledge()` 的流程变成：

```text
读取 knowledge/knowledge.txt
↓
调用 split_knowledge() 切分成 chunks
↓
提取用户问题关键词
↓
对每个 chunk 进行相关性评分
...
这个设计让 Agent 能明确知道检索是否成功。

---

### 四、agents/personal_agent.py 的改进

V9 中，知识库检索结果会通过通用的 `summarize_tool_result()` 进行总结。

V9.2 改进后，知识库问答不再走通用总结逻辑，而是新增了专门的知识库回答方法：

```text
answer_with_knowledge(user_input, knowledge_result)
```
...
用户输入知识库相关问题
↓
PersonalAgent.run_once(user_input)
↓
decide_tool() 判断应该调用 search_knowledge
↓
大模型返回 JSON 工具指令
↓
Python 调用 search_knowledge(query)
↓
knowledge_tool.py 读取 knowledge.txt
↓
split_knowledge() 切分知识库
↓
extract_keywords() 提取关键词
↓
score_paragraph() 给片段打分
↓
search_knowledge() 返回最相关片段
↓
answer_with_knowledge() 调用大模型
↓
模型基于检索结果生成回答
↓
返回给用户
...
Agent 能正确基于知识库回答 RAG 是 Retrieval-Augmented Generation，也就是检索增强生成。

我也测试了知识库中没有内容的问题：

```text
根据知识库回答：Transformer 的多头注意力机制是什么？
```

Agent 没有乱编，而是提示当前知识库中没有找到相关内容。
...
| 版本   | 能力        | 特点                      |
| ---- | --------- | ----------------------- |
| V9   | 简单本地知识库问答 | 读取 knowledge.txt，按关键词搜索 |
| V9.2 | 改进版本地 RAG | 改进切片、评分、排序，并基于检索结果回答    |

V9 更像是 RAG 的最小原型。

V9.2 开始接近真实 RAG 的核心流程：
...
```text
1. RAG 不是直接让大模型回答，而是先检索相关知识
2. 文档切分会影响检索效果
3. 检索结果需要排序，不能随便返回
4. Top-K 片段可以作为模型回答的上下文
5. 模型需要被限制只能基于上下文回答
6. 当知识库信息不足时，Agent 应该明确说明，而不是编造
```

这一步让我从“会调用大模型”进一步理解了“如何让大模型基于外部知识工作”。

---

### 九、V9.2 当前不足

V9.2 仍然是简单版 RAG，还有一些不足：

```text
1. 仍然是关键词匹配，不是真正语义检索
2. 没有使用 embedding
3. 没有向量数据库
4. 不能处理 PDF、Word 等复杂文档
5. 没有文档上传功能
6. 知识库内容仍然需要手动写入 knowledge.txt
7. 检索评分规则比较简单
```
...
V9.2 是改进版本地 RAG Agent，它在不引入新依赖的前提下，改进了知识库文本切分、关键词提取、片段评分和基于检索结果回答的流程，让 Agent 更接近真实 RAG 的工作方式。

---

### 十一、面试讲解版本

如果面试官问我 V9.2 做了什么，我可以这样回答：

> V9.2 是我在本地知识库问答 Agent 基础上做的一次 RAG 检索升级。我没有直接引入 LangChain 或向量数据库，而是先手写了一个简单 RAG 流程：读取本地 knowledge.txt，将文本切分成 chunks，提取用户问题关键词，对每个 chunk 做相关性评分，返回 top-k 片段，再让大模型只能基于这些片段回答。
> 这个版本让我理解了 RAG 的核心不是让模型凭空回答，而是先检索外部知识，再基于检索结果生成答案。
